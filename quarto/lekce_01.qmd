...
subtitle: "Od uspořádaného projektu k prvnímu reprodukovatelnému grafu"
---

## Co se naučíte

V této lekci si připravíte prostředí pro práci s daty, napíšete první příkazy a vytvoříte jednoduchý graf. Na konci byste měli umět rozlišit R od RStudia, založit projekt, uložit a spustit skript, vytvořit objekt a použít funkci. Vyzkoušíte si také, zda se váš výsledek podaří znovu získat po restartu R.

Předpokládáme nainstalované **R a RStudio Desktop** a základní orientaci v souborech a složkách. Programovat zatím umět nemusíte. Pro výklad grafu využijeme znalost bodového grafu a stručně připomeneme regresní přímku. Balíčky potřebné pro druhou část nainstalujeme samostatně; žádný vlastní datový soubor nepotřebujete.

Příklady s reakčními časy a dotazníkovými skóry jsou **uměle vytvořené pro výuku**, nejde o výsledky psychologického výzkumu. Část o různých regresních křivkách je nepovinná a můžete ji při prvním průchodu přeskočit.

## R počítá, RStudio pomáhá s prací

**R** je programovací jazyk a prostředí pro statistické výpočty a grafiku. **RStudio** je integrované vývojové prostředí (IDE): poskytuje editor, konzoli, prohlížení dat, grafů a nápovědy. Výpočet provádí R; RStudio usnadňuje jeho zadání a organizaci. R lze používat i bez RStudia. Základní R je freewarový software a další funkce do něj přidáváme prostřednictvím balíčků. [Zdroj: R Project – What is R?](https://www.r-project.org/about.html)
...
## Skript, čistá relace a pohodlné ovládání
...
## Balíčky: instalace není totéž co načtení (připojení)
...
**Připojení** balíčku pomocí `library()` umožňuje v aktuální relaci používat jeho funkce bez uvádění názvu balíčku. Běžně se mu říká také „načtení balíčku“. Příkazy `library()` zapisujeme na začátek skriptu, pokud v něm chceme funkce používat tímto způsobem. Pro následující graf spustíme:
...
`palmerpenguins` poskytuje výukový datový soubor s reálnými měřeními tučňáků, `ggthemes` použijeme pro paletu barev. Při `library(tidyverse)` se v konzoli mohou objevit informační zprávy o připojených balíčcích a konfliktech jmen funkcí. Nejsou automaticky chybou; hlášení `there is no package called ...` už ale znamená, že v dostupných knihovnách chybí požadovaný balíček.
...
Včera jste úspěšně nainstalovali `tidyverse`. Dnes jste restartovali R a chcete vytvořit graf. Který příkaz běžně potřebujete znovu spustit: `install.packages("tidyverse")`, nebo `library(tidyverse)`? Kam jej zapíšete?
...
### Cvičení 9: Vyměňte proměnnou na ose {.exercise}
...
Tučňáci Gentoo se v tomto souboru vykazují delší ploutve a vyšší hmotnosti. Při popisu vztahu bychom tedy můžeme rozlišovat rozdíly **mezi druhy** a analyzovat vztah mezi proměnnými i **uvnitř jednotlivých druhů**. Analogicky může v psychologii společný graf dvou skupin zachytit jak individuální souvislost, tak rozdíl skupinových průměrů.

### Nepovinně: co určuje počet a tvar regresních čar?

Funkce `geom_smooth()` přidává vyhlazenenou křivku pro zobrazení vztahu. V těchto datech použije metodu `loess`, která dovoluje zachytit i zakřivený průběh. Výchozí metoda závisí na velikosti největší skupiny: pod 1 000 pozorování se používá `loess`, u větších skupin standardně zobecněný aditivní model. Pro tuto ukázku je podstatné, jak umístění mapování ovlivní počet křivek. [Zdroj: ggplot2 – geom_smooth](https://ggplot2.tidyverse.org/reference/geom_smooth.html)
...
### Dokončený graf a opatrná interpretace
...
### Cvičení 10: Mapování, nebo pevné nastavení? {.exercise}
...
::: {.callout-note collapse="true" title="Ukázat řešení"}
Body se zmenší, ale zobrazené hodnoty ani odhad přímky se nezmění. `size = 1.5` je jednotné nastavení pro všechny body. `shape = species` je mapování: tvar závisí na konkrétní hodnotě proměnné, a proto uvnitř v `aes()`.

Jedna přímka vzniká proto, že barva a tvar podle druhu jsou nastavené jen v `geom_point()`. Vrstva `geom_smooth()` dědí z hlavního mapování pouze x a y. Přesunutí `color = species` do hlavního `aes()` by vedlo k několika samostatným přímkám podle druhu, jak ukazuje nepovinný příklad výše.
:::
...
Průměr fiktivních skórů je `12` bodů a výběrová směrodatná odchylka přibližně `3,39` bodu. Tyto údaje nesouvisejí s tabulkou tučňáků; jde o dvě oddělené části procvičení.
...
Po restartu běží skript znovu od `library()` až po export. Pokud se objeví `object ... not found`, zkontrolujeme chybějící řádek, překlep nebo pořadí příkazů. Jestliže chybí balíček, nejprve jej nainstalujeme do knihovny dostupné používané verzi R. Výsledkem mají být stejné číselné hodnoty a nově vytvořený obrázek.
:::
...
| Varování o odstraněných řádcích v grafu | Zkontrolujte chybějící či neplatné hodnoty zobrazených proměnných. Nepotlačujte hlášení bez porozumění. |
| Graf má správný titulek, ale jiná data | Popisky v `labs()` nemění mapování sloupců v `aes()`. Ověřte obojí. |
...
Graf sestavujeme z dat, mapování a vrstev. Popisky a jednotky nastavujeme kvůli srozumitelnosti; upozornění na chybějící data i rozdíl mezi skupinovým a celkovým vztahem patří k interpretaci. Nejdůležitější bod této lekce je praktický: uložte skript, restartujte R a ověřte, že výsledky vzniknou znovu.

## Doporučená četba

Další příklady a vysvětlení najdete v knize Wickhama, H., Çetinkaya-Rundel, M., a Grolemunda, G.: *R for Data Science*, 2. vydání. K tématům této lekce se vztahují kapitoly [1: Data visualization](https://r4ds.hadley.nz/data-visualize.html), [2: Workflow: basics](https://r4ds.hadley.nz/workflow-basics.html) a [6: Workflow: scripts and projects](https://r4ds.hadley.nz/workflow-scripts.html).
