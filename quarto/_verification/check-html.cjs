...
const result = { errors: [], screenshots: [] };
...
(async () => {
  ...
  try {
    ...
    check(result.solutions === 12, `Expected 12 solutions, got ${result.solutions}`);
    ...
    const links = await page.locator('main a[href^="#"]').evaluateAll(links => links.map(a => a.getAttribute('href')));
    for (const href of links) check(await page.evaluate(h => !!document.getElementById(decodeURIComponent(h.slice(1))), href), `Broken anchor ${href}`);
    ...
    await screenshot('html-top');
    await screenshot('html-workspace', '#fig-workspace');
    await screenshot('html-keyboard', '#specialni-znaky');
    await screenshot('html-headings', '#nadpisy-skriptu');
    await screenshot('html-copy', '.code-copy-outer-scaffold >> nth=1');
    await screenshot('html-expanded', '.callout .code-copy-outer-scaffold >> nth=0');
    ...
    console.log(JSON.stringify({ solutions: result.solutions, codeBlocks: result.codeBlocks, copyChecks: result.copyChecks, images: result.images.length, errors: result.errors, mobileWidth: result.mobileWidth }));
  } finally {
